...
|---|---|---|---|
| Q1 | **Delegating workshop-wide permissions to non-owner staff.** v1 keeps `manage_branches` / `manage_pricing` / `manage_settings` / `view_reports` and user management owner-only. Should a workshop be able to grant these to a "branch manager" staff user? | shape | When a workshop with > ~5 staff per branch asks for it, or when an owner reports being a bottleneck for pricing/branch edits. |
| Q2 | **Payment gateway integration** (Payme / Click / Uzum Bank) and the `pending_payment` → gateway-redirect flow. The seams exist (payment-channel flags + credentials are stored; `order_payment.method` has gateway values reserved); v1 records payments manually. | shape | when a workshop has a live merchant account ready. |
...
| Q5 | **SMS / email channels** and the **Telegram notification bot** for client notifications. v1 is in-app only. | shape | when client engagement metrics show the in-app-only inbox isn't read. |
...
| Q8 | **Order: batching, reorder, templates, partial fulfilment, complaint/return after `completed`, client ratings/feedback.** | shape | backlog grooming. |
...
That's the canon — now into the detail.

- [`ref/features/`](ref/features/) — per-feature working specs (orders, cutting,
  catalog-inventory, workshop, notifications, platform). Rules + UX per feature domain.
- [`ref/entities/`](ref/entities/) — one page per bounded context (sales, catalog, inventory,
  cutting, identity, workshop, support). Field shapes, states, invariants.
