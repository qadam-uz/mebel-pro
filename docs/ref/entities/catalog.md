...
| `name` | text | required, generated by the server and never an input; embeds manufacturer + decor + colour + trade dimensions, e.g. `LDSP Egger H1334 ST9 · Sonoma eman · 2750×1830×18 mm` or `Kromka Egger H1145 · Sonoma eman · 2×19 mm` |
...
| `edge_width_mm` | int? | **`edge` only**, required there; tape roll width in millimetres; null for `panel` |
| `image_file_id` | UUID? | → [file](support.md#file) — sample image |
| `status` | enum | `active` / `inactive` (soft delete only) |
| `created_at` / `updated_at` | timestamp | |

Invariants: `panel` materials have `type`, panel size (`length ≥ width`) and grain, and no
`edge_width_mm`; `edge` materials have none of the panel fields, require `edge_width_mm`,
and are stocked in integer millimetres; one standard panel size per `panel` material (v1) —
a material's identity is its spec, that size or tape width, **and its manufacturer**, so the
same spec in two manufacturers is two catalog rows, the same spec in two panel sizes is two
more, and one edge decor in two widths is two rows. The generated `name` embeds the
manufacturer deliberately because order-line snapshots, logs, and PDFs often carry only
`material.name`; generation prevents hand-written drift. Materials are created and edited only by a platform operator (platform
