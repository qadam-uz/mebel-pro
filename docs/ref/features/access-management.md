...
| `process_production`   | the **cutter & edger workspaces** — see orders assigned to this user, view the cutting plan read-only, mark **Cutting done** (→ `edge_banding` or `ready`; stamps the cutter snapshot, decrements panel stock for `shop` panels) and **Banding done** (→ `ready`; stamps the edge snapshot, decrements edge stock per edge material for `shop` sides). Cannot edit, verify, cancel, or revert an order. Those workspaces exist only on a branch running **full** production mode; on a `simple` branch the whole grant is dormant — production is closed by `manage_orders`'s one **Tayyor** tap and this grant only makes the holder nameable as the credited worker ([`orders.md`](orders.md#production-mode)). |
