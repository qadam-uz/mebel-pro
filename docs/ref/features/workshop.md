...
- **Branches list** (`/workshop/branches`) — simple table: name, address, primary phone, status
  badge, action. **+ Branch** (owner). Empty: "No branches yet — add one to start taking orders."
...
- **Branch detail** (`/workshop/branches/:id`) — owner-only editable branch form: branch
  contact, pricing (entered in so'm), cutting settings (kerf + edge trim, in mm), and status
  controls. It does not duplicate materials, stock, staff, or order management; those stay in
