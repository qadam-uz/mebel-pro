...
| `compose.prod.yaml` | **Prod**, self-contained. No local data services — backend joins the external `infra-net` Docker network and reaches the shared `postgres` + `minio` services by name. `backend` runs the built image (alembic upgrade on start), `web` is the built SPA served by nginx, and a **Caddy edge** (built from `edge.Dockerfile`, Caddyfile baked in) is the only published service (80 / 443 / 443-UDP; auto-HTTPS per host). Routes by subdomain derived from `BASE_DOMAIN`: apex → landing; `app.*` / `workshop.*` / `admin.*` → web; `/api/*` on every SPA subdomain → backend; on `admin.*` also `/docs`, `/api-docs`, `/api-redoc` → backend (HTTP Basic-gated). Includes per-service log rotation and resource caps. Bucket creation is **not** done here — provision the `MINIO_BUCKET` on the shared MinIO once via its console / `mc`. |
