  ...
  <section>
    ...
    <div class="page-head">
      <div>
        ...
        <!-- The branch number is the last segment of the branch's own QR link
             (`/w/{code}/{branch_no}`); this is the only place it can be looked
             up. It no longer appears in order numbers — those are six random
             platform-wide digits (docs/ref/entities/sales.md). -->
        <i18n-t
    ...
    <template v-else-if="workshop.selectedBranch">
      ...
      <!-- The artifact the counter needs, above the settings form: the owner
           comes to this screen for the QR far more often than for the kerf. -->
      <WorkshopLinkCard
        class="mb-5"
        :code="workshop.selectedBranch.workshop_public_code"
        :branch-no="workshop.selectedBranch.branch_no"
        :print-to="rolePath(`/workshop/branches/${workshop.selectedBranch.id}/client-link`)"
      />
