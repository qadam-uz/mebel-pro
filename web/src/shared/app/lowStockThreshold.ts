...
// that first pass lie about both. A 0 threshold means the row is **not watched
// at all** — never low, never counted on the Asosiy card — which is the honest
// state for a format registered before anyone decided how much of it to keep.
// The row stays editable the moment the branch knows the number (from the
// catalog form or from the stock detail), and existing branch materials keep
// what they were saved with.
//
// A function, not a `const`, so a future per-unit default can take `type` back
...
export function thresholdUnit(type: DecorType | null | undefined): string {
  return formatStockUnit(isTape(type) ? 'metre' : 'panel')
}
