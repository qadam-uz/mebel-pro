...
// Full paginated notifications list — read/unread filter + mark-all-read.
// Order notifications deep-link to the workshop order. Mirrors notifications.html.
...
const emptyMsg = computed(() => {
  if (filter.value === 'unread') return t('workshop.notifEmptyUnread')
  if (filter.value === 'read') return t('workshop.notifEmptyRead')
  return t('workshop.notifEmptyAll')
})
...
async function open(n: Notification) {
  ...
  if (n.entity_type === 'order' && n.entity_id) router.push(`/workshop/orders/${n.entity_id}`)
}
  ...
  <div>
    <div class="page-head">
      <div>
        ...
        <p class="sub">{{ t('workshop.notificationsSub') }}</p>
      </div>
      <div class="tools">
