...
// Operator self-profile — read-only identity, change password (strength meter),
// active sessions (revoke / logout-all), and the gated docs/API-reference links.
...
function deviceLabel(s: SessionInfo): string {
  ...
  return (info.user_agent as string)?.slice(0, 48) || (info.ip as string) || '—'
}
...
const docsLinks = [
  { href: '/docs', label: t('admin.docsLive') },
  { href: '/api-docs', label: t('admin.apiSwagger') },
  { href: '/api-redoc', label: t('admin.apiRedoc') },
]
...
<template>
  <div>
    ...
    <div style="display: grid; gap: 18px; max-width: 720px">
      ...
      <section class="card">
        <div class="card-h">
          <h2>{{ t('admin.docsRefTitle') }}</h2>
        </div>
        <div class="card-b">
          <p class="hint" style="margin: 0 0 10px">{{ t('admin.docsRefNote') }}</p>
          <div class="flex flex-wrap">
            <a
              v-for="l in docsLinks"
              :key="l.href"
