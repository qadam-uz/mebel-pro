...
test('admin provisions and blocks a workshop', async ({ page }, testInfo) => {
  ...
  await page
    ...
    .click()
  await expect(page.getByRole('heading', { name: `Workshop ${id}` })).toBeVisible()
  await page.getByRole('button', { name: new RegExp(`Workshop ${id}.*bloklash`) }).click()
  ...
  await page.getByRole('button', { name: /ustaxonasini blokdan chiqarish/ }).click()
