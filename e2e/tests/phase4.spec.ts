...
test('client signs in with Telegram OTP, optimizes a cutting draft, and downloads PDF', async ({
  ...
  await page.getByRole('button', { name: 'Continue' }).click()
  await page.getByLabel('Name').fill('Phase 4 Client')
  await page.getByRole('button', { name: 'Continue' }).click()
  ...
  await expect(page.getByRole('heading', { name: 'Cutting editor' })).toBeVisible()

  await page.getByRole('button', { name: 'Pick a branch' }).click()
  await page.getByRole('button', { name: 'Preferred branch' }).click()
  ...
  await expect(page.getByText('Edge split')).toBeVisible()
  await expect(page.getByRole('button', { name: /Use this one|Chosen/ })).toHaveCount(2)
