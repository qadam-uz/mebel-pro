...
test('client browses public branch catalog without stock details', async ({ page, request }, testInfo) => {
  ...
  await page.getByLabel('Phone').fill(phoneFor(id, 60))
  await page.getByRole('button', { name: 'Send code' }).click()
  await page.getByLabel('Code').fill('000000')
  await page.getByRole('button', { name: 'Continue' }).click()
  await page.getByLabel('Name').fill('Phase 3 Client')
  await page.getByRole('button', { name: 'Continue' }).click()
  await page.getByRole('link', { name: 'Branches' }).first().click()
  await page.getByLabel('Search branches').fill(`Phase 3 Workshop ${id}`)
  await page.getByRole('button', { name: 'Search' }).click()

  await expect(page.getByRole('heading', { name: 'Branches and materials' })).toBeVisible()
  await expect(page.getByRole('heading', { name: `Phase 3 Workshop ${id}` })).toBeVisible()
  await expect(page.getByText(material.name)).toBeVisible()
  await expect(page.locator('img[src^="blob:"]')).toBeVisible()
  await expect(page.getByText(/UZS\s*2[, ]500/)).toBeVisible()
