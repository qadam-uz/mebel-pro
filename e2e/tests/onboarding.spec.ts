...
test("system leads a fresh owner from temp password to an orderable workshop", async ({
  ...
  await page.getByLabel("Kesish narxi (so'm)").click();
  await expect(pricingHint).toHaveCount(0);
  await page.getByLabel("Kesish narxi (so'm)").fill("25000");
  await page.getByLabel("Kromka yopishtirish narxi (so'm)").fill("5000");
  const editForm = page
